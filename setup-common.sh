...
show_ubuntu_next_steps() {
    ...
    echo "  2. From your local machine, sync deployment files:"
    echo "     rsync -avz --exclude 'secrets/' ./ vaki-lenovo:~/openclaw/"
    echo "  3. Initialize secrets on this server:"
    echo "     ./init-secrets.sh"
    echo "  4. From your local machine, build and push to Docker Hub:"
    echo "     ./build-remote.sh --push --tag v1.0.0"
    echo "  5. Deploy on this server from Docker Hub:"
    echo "     USE_DOCKERHUB=true ./deploy.sh v1.0.0"
    echo ""
    echo "Or deploy latest version:"
    echo "     USE_DOCKERHUB=true ./deploy.sh"
    echo ""
    echo "Manual all-in-one deployment:"
    echo "  rsync -avz --exclude 'secrets/' ./ vaki-lenovo:~/openclaw/ && \\"
